...
class KeyboardModeDict(OrderedDict):
    """Ordered mapping of keyboard name -> KeyboardMode, with JSON-like str()."""

    def __str__(self):
        return json.dumps(
            OrderedDict((k, str(v)) for k, v in self.items()), sort_keys=True
        )


def keyboard_modes_dict(value):
    """Convert a configuration value to KeyboardModeDict.

    Accepts either a KeyboardModeDict instance or a JSON string mapping
    keyboard names to mode strings.
    """

    if isinstance(value, KeyboardModeDict):
        return value
    if isinstance(value, str):
        return KeyboardModeDict(
            (k, KeyboardMode(v)) for k, v in json.loads(value).items()
        )
...
class Keyboard(StenotypeBase):
    ...
    async def _send_stroke(self, index: int, stroke_on_hold, stroke_on_release):
        # Fire after a small delay if the state is still the same.
    ...
    def start_capture(self):
        ...
        self._initializing()
        # See _check_special_actions/_send_stroke for how this index is used.
        ...
        # Special actions mapped from held chords to (stroke_on_hold, stroke_on_release).
        ...
        for combo in itertools.product(
            ...
            s = sum(combo, Stroke("PWR*"))
            self._special_actions[s] = (s | Stroke("-FBLSD"), s | Stroke("-RPGTZ"))
            # starboard variant
            self._special_actions[s - Stroke("*") + Stroke("#")] = (
                s | Stroke("-FBLSD"),
                s | Stroke("-RPGTZ"),
            )
        ...
        try:
            ...
            # Hook ready/error callbacks from Capture.
    ...
    def suppress_last_stroke(self, send_backspaces):
        # Suppression is handled via special actions; nothing to do here.
        pass
    ...
    def _check_special_actions(self):
        stroke = self._keys_to_stroke(self._down_keys)
        if stroke in self._special_actions:
            stroke_on_hold, stroke_on_release = self._special_actions[stroke]
